...
class ExcelImporter {
  ...
  parseContactsData(rawData) {
    ...
    try {
      ...
      const unions = [];
      const unionIndex = new Map(); // key: sorted partner ids -> Union

      contacts.forEach((contact) => {
        if (Array.isArray(contact.managerIds) && contact.managerIds.length > 0) {
          const partners = Array.from(new Set(contact.managerIds.map(String)));
          partners.sort();
          const key = partners.join('|');
          let u = unionIndex.get(key);
          if (!u) {
            u = new Union({ partnerIds: partners, childIds: [], type: 'coparent' });
            unions.push(u);
            unionIndex.set(key, u);
          }
          if (!u.childIds.includes(contact.id)) u.childIds.push(contact.id);

          // annotate partners and child with union refs
          partners.forEach(pid => {
            const p = contacts.find(c => c.id === pid);
            if (p) {
              p.unionIds = p.unionIds || [];
              if (!p.unionIds.includes(u.id)) p.unionIds.push(u.id);
            }
          });

          contact.parentUnionIds = contact.parentUnionIds || [];
          if (!contact.parentUnionIds.includes(u.id)) contact.parentUnionIds.push(u.id);
        }
      });

      // Attach unions metadata non-intrusively to the returned array
      if (unions.length > 0) {
        contacts.__unions = unions.map(u => u.toJSON());
        console.log('[ExcelImporter] ℹ️ Unions créées:', contacts.__unions.length);
      }
    } catch (e) {
      console.error('[ExcelImporter] Error building unions:', e);
    }
