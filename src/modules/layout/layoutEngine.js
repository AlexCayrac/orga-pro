const assignLevels = require('./assignLevels');
const barycenterOrdering = require('./barycenterOrdering');
const { sweepCrossingReduction, countCrossings } = require('./crossingReduction');
const positioning = require('./positioning');
const { computeLayoutStats } = require('./utils');

class LayoutEngine {
  constructor(graphModel, opts = {}) {
    this.graphModel = graphModel;
    this.options = Object.assign({ sweepPasses: 8 }, opts || {});
  }

  computeFullLayout(runOptions = {}) {
    const opts = Object.assign({}, this.options, runOptions || {});

    // 1. assign levels
    const { personLevels, unionLevels } = assignLevels(this.graphModel, opts.assign || {});

    // 2. initial ordering
    const orderByLevel = barycenterOrdering(this.graphModel, { personLevels, unionLevels });

    // 3. measure crossings before -> apply sweep reduction -> measure after
    const before = countCrossings(orderByLevel, this.graphModel);
    const passes = Number(opts.sweepPasses || 8) || 8;
    const reduced = sweepCrossingReduction(orderByLevel, this.graphModel, passes);
    const after = countCrossings(reduced, this.graphModel);
    console.log(`[Layout] crossings:`, before, '→', after);

    // 4. positioning
    const positions = positioning(this.graphModel, reduced, { personLevels, unionLevels }, opts.positioning || {});

    const stats = computeLayoutStats(positions, this.graphModel);

    return { positions, stats, personLevels, unionLevels, orderByLevel: reduced, crossingsBefore: before, crossingsAfter: after };
  }
}
 ...
 * }
 */
function calculateOrgChartLayout(blocks, contacts, canvasWidth = null, canvasHeight = null) {
  console.log(`\n[LAYOUT-ENGINE] ════════════════════════════════════════════`);
...
module.exports.calculateOrgChartLayout = calculateOrgChartLayout;
module.exports.LAYOUT_CONFIG = LAYOUT_CONFIG;
