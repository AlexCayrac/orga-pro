...
class ExcelLoader {
  ...
  async loadEmbeddedExcel() {
    try {
      ...
      // Ensure we have a hash even if the IPC didn't provide one
      let finalHash = result.hash;
      try {
        if (!finalHash) {
          finalHash = this.calculateHashFromData(normalized || data || []);
          logger.info('[ExcelLoader] Calculated fallback hash:', finalHash);
        }
      } catch (hErr) {
        logger.warn('[ExcelLoader] Failed to compute fallback hash:', hErr.message || hErr);
      }
